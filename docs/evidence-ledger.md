...
- PostgreSQL schema/repository integration：`SENTINEL_POSTGRES_ADMIN_URL=... python -m pytest -q apps/control-api/tests/test_postgres_integration.py -m integration` 在本机 PostgreSQL 15 临时数据库验证 migration up/重复 up、Incident/Timeline/Approval/Outbox 写入与去重、Alert Ingress nonce 原子 claim/过期清理、workflow checkpoint 创建/更新/重启读取、一次性审批决定、dispatcher 发布、Control API full lifespan、PostgresStore 重启后 Incident/Timeline/Approval 读模型重建、外部连接追加事件后的权威时间线读取，以及幂等响应记录重开读取。Action Gateway 集成另验证 ActionExecution 重启后保留 runbook_ref 和 opaque idempotency key。`scripts/verify_postgres_migrations.py` 另行验证 11 张业务表、索引/append-only trigger、active fingerprint 唯一约束、nonce 唯一约束、down/reapply，并在 finally 删除临时库。该证据仍不覆盖真实 Temporal crash/restart 对账或 Alertmanager/观测栈联调。
...
- SSE replay 窗口切片：Control API 以 `SENTINEL_SSE_REPLAY_MAX_EVENTS` 限制服务端补发范围，过期 `Last-Event-ID` 返回 `410/SSE_CURSOR_EXPIRED` 及最早 sequence；PostgreSQL repository 在本机真实数据库验证 sequence 边界查询，流轮询持续从权威时间线读取。当前没有低延迟跨进程 broker、慢客户端背压或物理审计清理。
- Temporal/DB reconciliation：Workflow 结果记录调查、动作、验证 `workflow_event_refs`；新增注册到 Temporal Worker 的 `reconcile_postgres_projection` Activity，从 Worker 环境读取 `DATABASE_URL`、查询 PostgreSQL Incident projection，并复用 checkpoint reconciler 拒绝 workflow run/event、状态和落后 projection version 漂移；本机 PostgreSQL 集成通过。各业务投影事务尚未全部写入 workflow checkpoint，完整 crash/restart 对账仍未完成。
- Temporal 审批 Signal outbox 切片：full Control API 启动时连接 Temporal，dispatcher 从 PostgreSQL 领取 `approval.decided`，通过主事件循环投递带 approval ID、plan hash、决定人和 expiry 的 `approval_decision` Signal；Signal 失败不标记 outbox 已发布。控制 API 的 payload/线程桥接单测，以及 SDK 测试服务器中 Workflow 等待审批到 `RESOLVED` 的端到端测试通过；尚未覆盖真实多场景 Workflow 启动、投影 checkpoint 全链路和网络分区。
- Workflow checkpoint 持久化切片：PostgreSQL 新增 `workflow_checkpoints` 表，full `PostgresStore` 的创建、更新、可恢复列表和重启读取已通过本机集成测试；checkpoint 只保存恢复引用，Temporal history、跨业务投影全链路写入和 crash/restart 对账仍未完成。
- Full profile 安全边界：Control API 在 `SENTINEL_PROFILE=full` 时执行 PostgreSQL health check、migration，使用 PostgreSQL Incident/Timeline/Approval 写入，并在启动时重建读模型；缺少 DATABASE_URL、驱动或数据库不可达仍显式 fail-closed，且拒绝本地 Scenario Runner 及所有 `LocalExerciseWorkflow` checkpoint/审批续跑，防止误用本地 SQLite 或合成恢复结果冒充 full。
