...
- PostgreSQL schema/repository integration：`SENTINEL_POSTGRES_ADMIN_URL=... python -m pytest -q apps/control-api/tests/test_postgres_integration.py -m integration` 在本机 PostgreSQL 15 临时数据库验证 migration up/重复 up、Incident/Timeline/Approval/Outbox 写入与去重、一次性审批决定、dispatcher 发布、Control API full lifespan、PostgresStore 重启后 Incident/Timeline/Approval 读模型重建、外部连接追加事件后的权威时间线读取，以及幂等响应记录重开读取。Action Gateway 集成另验证 ActionExecution 重启后保留 runbook_ref 和 opaque idempotency key。`scripts/verify_postgres_migrations.py` 另行验证 9 张表、索引/append-only trigger、active fingerprint 唯一约束、down/reapply，并在 finally 删除临时库。该证据仍不覆盖真实 crash/restart 对账。
...
- Incident Worker telemetry adapter 切片：full profile 的 Prometheus/Loki/Tempo Activity 已通过受限 `HttpTelemetrySource` 执行启动时配置的 HTTP 查询，返回 `source_mode=observed` 和 payload hash；SLO Activity 解析 Prometheus p99（秒转毫秒）并按 observed window 判定；Action Activity 已通过带时间窗 HMAC 服务签名调用 `/api/actions` 并要求审批/目标身份字段，Temporal ActionRequest 已传递 plan hash、审批过期时间和目标身份。缺任一来源或后端失败即拒绝，light profile 仍保留 fixture。当前 Kubernetes 只读和 Temporal/DB 全链路 ActionExecution 对账仍未完成。
