...
- PostgreSQL schema/repository integration：`SENTINEL_POSTGRES_ADMIN_URL=... python -m pytest -q apps/control-api/tests/test_postgres_integration.py -m integration` 在本机 PostgreSQL 15 临时数据库通过 `4 passed`；真实验证 migration up/重复 up、domain Incident/Timeline/Outbox 写入与去重、dispatcher 发布、Control API full lifespan、PostgresStore 重启后 Incident/Timeline 读模型重建，以及外部连接追加事件后的权威时间线读取。`scripts/verify_postgres_migrations.py` 另行验证 8 张表、索引/append-only trigger、active fingerprint 唯一约束、down/reapply，并在 finally 删除临时库。该证据仍不覆盖审批/Action PostgreSQL 接线或真实 crash/restart 对账。
