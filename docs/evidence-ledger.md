...
- Incident Worker telemetry adapter 切片：full profile 的 Prometheus/Loki/Tempo Activity 已通过受限 `HttpTelemetrySource` 执行启动时配置的 HTTP 查询，返回 `source_mode=observed` 和 payload hash；SLO Activity 解析 Prometheus p99（秒转毫秒）并按 observed window 判定；缺任一来源或后端失败即拒绝，light profile 仍保留 fixture。当前 Kubernetes 只读和 Action Gateway HTTP 调用仍未完成。
