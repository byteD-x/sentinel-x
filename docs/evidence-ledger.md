...
- PostgreSQL domain repository slice：新增 `PostgresIncidentRepository`，将 active fingerprint 去重、Incident/Timeline/Outbox 初始写入、序号锁定和 ApprovalDecision 一次性事务边界固化；fake DB 单元测试覆盖 SQL 写入顺序。真实 psycopg repository 集成待驱动可安装后执行，当前不升级为 full claim。
- PostgreSQL outbox dispatcher slice：新增 `PostgresOutboxDispatcher`，使用 `FOR UPDATE SKIP LOCKED`、aggregate/sequence 顺序、成功 `published_at` 和失败 `attempt_count/last_error/available_at` 重试；fake DB 单元测试覆盖成功、失败和配置边界。尚未接入 Control API lifespan 或真实跨进程 crash/restart 测试。
