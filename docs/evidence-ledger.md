...
- Incident Worker telemetry adapter 切片：full profile 的 Prometheus/Loki/Tempo Activity 已通过受限 `HttpTelemetrySource` 执行启动时配置的 HTTP 查询，返回 `source_mode=observed` 和 payload hash；缺任一来源或后端失败即拒绝，light profile 仍保留 fixture。当前 Kubernetes 只读、Action Gateway HTTP 调用和真实 SLO 窗口仍未完成。
