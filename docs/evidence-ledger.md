...
- 本地门禁：`python -m pytest -q`，2026-08-24（Windows，本地 light）结果为 `170 passed`；新增 Alert Ingress nonce replay 拒绝、审批 policy/plan hash 服务器端重算、非法目标/篡改、工作流恢复、六场景 fixture evaluator 和 SLO 观测窗口测试，但仍未形成固定 benchmark。
