...
- PostgreSQL schema/repository integration：`SENTINEL_POSTGRES_ADMIN_URL=... python -m pytest -q apps/control-api/tests/test_postgres_integration.py -m integration` 在本机 PostgreSQL 15 临时数据库验证 migration up/重复 up、Incident/Timeline/Approval/Outbox 写入与去重、Alert Ingress nonce 原子 claim/过期清理、一次性审批决定、dispatcher 发布、Control API full lifespan、PostgresStore 重启后 Incident/Timeline/Approval 读模型重建、外部连接追加事件后的权威时间线读取，以及幂等响应记录重开读取。Action Gateway 集成另验证 ActionExecution 重启后保留 runbook_ref 和 opaque idempotency key。`scripts/verify_postgres_migrations.py` 另行验证 10 张表、索引/append-only trigger、active fingerprint 唯一约束、nonce 唯一约束、down/reapply，并在 finally 删除临时库。该证据仍不覆盖真实 crash/restart 对账或 Alertmanager/观测栈联调。
