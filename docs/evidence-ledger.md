...
- Full profile 安全边界：Control API 在 `SENTINEL_PROFILE=full` 时执行 PostgreSQL health check、migration，使用 PostgreSQL Incident/Timeline/Approval 写入，并在启动时重建读模型；缺少 DATABASE_URL、驱动或数据库不可达仍显式 fail-closed，且拒绝本地 Scenario Runner/fixture checkpoint 自动续跑，防止误用本地 SQLite 或合成恢复结果冒充 full。
