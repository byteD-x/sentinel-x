...
- full 多场景 Temporal replay、PostgreSQL projection/outbox 和 full observability E2E 未完成；Control API、Diagnostic Gateway 和 Incident Worker 现在会在缺少真实适配器时 fail-closed，禁止静默回退到本地 SQLite 或 fixture 成功。当前 outbox 仅为 local SQLite 可恢复切片。
