...
- Action Gateway 已默认 fail-closed，并校验 HMAC 审批凭证、audience、管理员令牌、独立审批记录、目标身份、一次性消费和 full profile control-api 服务签名；新增按 opaque 幂等键的超时协调查询及基于原 approval 的 fake K8s reconcile，full PostgreSQL ActionExecution 登记使用唯一幂等键原子冲突处理并复用既有执行，light SQLite backend 与 full PostgreSQL 权威审批读取/条件消费、ActionExecution 重启元数据恢复均已覆盖验证，仍缺正式 TokenReview/mTLS、跨服务事务和真实网络分区 E2E。
...
- light Workflow fixture 的动作执行仍是模拟路径，但恢复结论已改为读取受控观测样本；full Control API 明确拒绝本地 Scenario Runner 与所有 `LocalExerciseWorkflow` checkpoint/审批续跑，避免 fixture 事件进入 PostgreSQL full 路径；另有单场景 `SentinelIncidentWorkflow` 已注册真实 Temporal Worker，并通过 SDK 测试服务器执行、Signal、Worker restart 和 history replay。
- full 多场景 Temporal replay、Kubernetes 真实观测查询和 full observability E2E 未完成；Prometheus/Loki/Tempo Activity、固定 namespace 的 Kubernetes PodList Activity 已接入受限 HTTP 来源，SLO p99 已按真实响应解析，Action Activity 已接入带服务签名的 Gateway HTTP 调用，并在缺配置时 fail-closed。PostgreSQL schema migration、domain repository、Incident/Timeline/Approval/ActionExecution/VerificationResult/workflow checkpoint 持久化、outbox dispatcher、Control API lifespan 后台发布、Action Gateway PostgreSQL 审批消费、Temporal reconciliation Activity、full 审批决定 outbox 到 Temporal Signal、跨连接时间线读取及带 `retry`/heartbeat、replay 窗口和 `410` 过期 cursor 的 SSE 游标续接已在本机验证，业务投影全链路 checkpoint 写入、真实 RBAC/crash/restart 对账和低延迟跨进程 SSE 发布仍未完成。Control API、Action Gateway、Diagnostic Gateway 和 Incident Worker 禁止静默回退到本地 SQLite 或 fixture 成功。
