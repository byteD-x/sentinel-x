...
- light Workflow fixture 的动作执行仍是模拟路径，但恢复结论已改为读取受控观测样本；full Control API 明确拒绝本地 Scenario Runner 与 checkpoint 自动续跑，避免 fixture 事件进入 PostgreSQL full 路径；另有单场景 `SentinelIncidentWorkflow` 已注册真实 Temporal Worker，并通过 SDK 测试服务器执行、Signal、Worker restart 和 history replay。
