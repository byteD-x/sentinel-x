...
- Action Gateway 已默认 fail-closed，并校验 HMAC 审批凭证、audience、管理员令牌、独立审批记录、目标身份、一次性消费和 full profile control-api 服务签名；light SQLite backend 与 full PostgreSQL 权威审批读取/条件消费、ActionExecution 重启元数据恢复均已覆盖验证，仍缺正式 TokenReview/mTLS 和 ActionExecution 跨服务事务。
