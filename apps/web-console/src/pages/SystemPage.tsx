  ...
  return (
    <div className={styles.page}>
      <header className={styles.header}>
        <div><div className={styles.eyebrow}><ServerCog size={14} aria-hidden="true" /> 运行环境</div><h1>运行环境</h1><p>确认控制面连接和恢复操作开关。所有数据均与生产环境隔离。</p></div>
      ...
      {error ? <div className={styles.error} role="alert"><CircleAlert size={17} aria-hidden="true" />{error}</div> : <div className={styles.grid}>
        ...
        <StatusRow label="运行模式" value="本地演练" ok />
      ...
      <div className={styles.safety}><LockKeyhole size={17} aria-hidden="true" /><span>高风险操作已禁用，系统不会向生产环境写入数据。</span></div>
    </div>
  )
}
