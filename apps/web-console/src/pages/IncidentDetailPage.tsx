...
import { AlertTriangle, ArrowLeft, ArrowRight, CheckCircle2, Clock3, FileCheck2, PlayCircle, Search, ShieldCheck, X } from 'lucide-react'
...
const EVENT_LABELS: Record<string, string> = {
  ...
  'evidence.collected': '调查证据', 'hypothesis.generated': '待验证判断', 'plan.proposed': '恢复方案',
  ...
  return (
    <div className={styles.page}>
      ...
      <section className={styles.summaryGrid}>
        ...
        <div className={`${styles.card} ${styles.diagnosisCard}`}><div className={styles.diagnosisTitle}><Search size={15} aria-hidden="true" /><span>待验证判断</span></div>{latestHypothesis ? <><strong className={styles.diagnosisStatement}>{readableEventValue(latestHypothesis.payload.statement, '等待调查证据')}</strong><div className={styles.diagnosisMeta}><span>{serviceLabel(latestHypothesis.payload.affected_service || '相关服务')}</span><b>置信度 {formatConfidence(latestHypothesis.payload.confidence)}</b></div><div className={styles.confidenceTrack} aria-label="待验证判断置信度"><span style={{ width: `${confidencePercent(latestHypothesis.payload.confidence)}%` }} /></div></> : <span className={styles.empty}>等待调查证据</span>}</div>
      </section>
